...
// огранённый камень. Первая попытка сделать градиент была почти незаметна
// (белый блик поверх и так белой звезды на значке в 15-20px — разницы не
// видно). Теперь — настоящий контраст: низ звезды уходит в заметно более
// тёмный синий (реальная тень грани, не полутон), а сверху слева — отдельная
// чёткая искра-блик (не растянутый по всей звезде градиент), как у бликов на
// огранённых камнях в иконках Telegram/App Store.
//
// Градиент/искра — через <defs> с id, а id в инлайновом SVG виден всему
...
<svg viewBox="0 0 24 24" aria-hidden="true" class="ps-star">
  <defs>
    <linearGradient id="ps-fill-${uid}" x1="0" y1="0" x2="0.3" y2="1">
      <stop offset="0%" stop-color="#ffffff"/>
      <stop offset="45%" stop-color="#ffffff"/>
      <stop offset="100%" stop-color="#8fa8f5"/>
    </linearGradient>
  </defs>
  <path d="M12 2.6l2.7 5.9 6.4.7-4.8 4.4 1.3 6.3L12 16.7 6.4 19.9l1.3-6.3-4.8-4.4 6.4-.7z"
    fill="url(#ps-fill-${uid})" stroke="#ffffff" stroke-width="0.4" stroke-opacity="0.6"/>
  <!-- Искра-блик: отдельный четырёхлучевой ромб, а не градиент по всей
       звезде — так его видно даже на 15px, где тонкий полутон теряется. -->
  <path d="M8.3 5.4c.25 1 .95 1.7 1.95 1.95-1 .25-1.7.95-1.95 1.95-.25-1-.95-1.7-1.95-1.95 1-.25 1.7-.95 1.95-1.95Z" fill="#fff"/>
</svg>`;
}
