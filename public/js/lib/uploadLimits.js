...
export const UPLOAD_LIMITS = {
  // Видео перед отправкой пережимается на клиенте (см. lib/video.js) до ~480p;
  // предел здесь — отсечь заведомо неподъёмное (длинные ролики, которые
  // пережатие пропускает) до начала передачи.
  video: 300 * MB,
  image: 25 * MB,
  file: 12 * MB,
  voice: 8 * MB,
  // Трёхминутный кружок на пониженном битрейте (см. recorder.js) — около 16 МБ;
  // 20 оставляет запас на разброс кодеков между браузерами.
  "video-note": 20 * MB,
  avatar: 2 * MB,
  "avatar-video": 3 * MB,
};
