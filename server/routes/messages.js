...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (chat.type === "dm") {
      ...
      // Администратору пишут как всем остальным.
      //
      // Здесь стоял запрет «в чат администрации нельзя писать напрямую». Он
      // задумывался ради того, чтобы заявки на покупку создавались только
      // автоматически, но побочно превращал администратора в аккаунт, которому
      // нельзя написать вообще — ни с вопросом, ни с жалобой.
      //
      // Заявки по-прежнему создаются сами (routes/premium.js и соседние шлют их
      // через lib/systemChat.js), а обычное письмо теперь доходит.
